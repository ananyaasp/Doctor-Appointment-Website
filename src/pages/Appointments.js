...
const Appointments = () => {
  ...
  const handleBooking = async (location, index) => {
    ...
    // Temporarily block out the slot in the UI
    const newSlots = { ...timeSlots };
    newSlots[location][index].available = false;
    setTimeSlots(newSlots);

    try {
      // Replace with the actual doctor and patient ID in your application
      const response = await fetch("http://localhost:3000/api/book-appointment", {
        ...
        body: JSON.stringify({
          ...
          date: "2024-11-15", // Replace with actual date or dynamic date from the calendar
          time: slot.time,
        }),
      });
  ...
  return (
    <div className="appointments-container">
      ...
      {/* Button for date selection, this can be linked to a date picker */}
